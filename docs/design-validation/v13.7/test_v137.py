"""Testes da revisão v13.7 (escritos ANTES de models_v137.py).

Uso: python test_v137.py <modulo>:<Classe> [arquivo_resultado.json]
Classificação:
  PASS         comportamento exigido observado
  FAIL         a API existe e o comportamento diverge (falha comportamental real)
  API_AUSENTE  o modelo não tem o método/coluna exigido (não é falha comportamental)
Não toca InstaT, Android, Instagram nem proxy.
"""
import datetime
import importlib
import json
...
sys.path.insert(0, os.path.join(os.path.dirname(HERE), "v136"))
...
def c01(M):
    ...
    code = ("import sys; sys.path.insert(0, r'%s'); import %s as mm; "
            "m = mm.%s(r'%s', create=False, busy_timeout=10.0); "
...
def main():
    spec = sys.argv[1]
    out_name = sys.argv[2] if len(sys.argv) > 2 else "results.json"
    modname, clsname = spec.split(":")
    M = getattr(importlib.import_module(modname), clsname)
    started = datetime.datetime.now().isoformat(timespec="seconds")
    rows, counts = [], {"PASS": 0, "FAIL": 0, "API_AUSENTE": 0}
    print(f"=== {getattr(M, 'name', spec)} | início {started}")
    for tid, title, fn in TESTS:
        t0 = time.perf_counter()
        try:
            ok, detail = fn(M)
            status = "PASS" if ok else "FAIL"
        except ApiAusente as e:
            status, detail = "API_AUSENTE", str(e)
        except AttributeError as e:
            status, detail = "API_AUSENTE", f"AttributeError: {e}"
        except Exception as e:  # noqa: BLE001
            status = "FAIL"
            detail = f"EXCEÇÃO {type(e).__name__}: {e} | {traceback.format_exc().strip().splitlines()[-3].strip()}"
        dt = time.perf_counter() - t0
        counts[status] += 1
        print(f"{tid:5} {status:11} {dt:5.2f}s  {title}\n      {detail}")
        rows.append({"id": tid, "title": title, "status": status, "detail": detail, "seconds": round(dt, 3)})
    print(f"--- {getattr(M, 'name', spec)}: {counts}")
    with open(os.path.join(HERE, out_name), "w", encoding="utf-8") as f:
        json.dump({"model": getattr(M, "name", spec), "started_at": started, "python": sys.version.split()[0],
                   "sqlite": sqlite3.sqlite_version, "counts": counts, "tests": rows}, f, ensure_ascii=False, indent=2)
