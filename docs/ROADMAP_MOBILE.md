...
### 3.2 Decisão de arquitetura: substituir todo o Selenium por Playwright? **Não.**

**Pergunta (Tiago, 14/09/2026):** vale trocar tudo de Selenium por Playwright?

**Análise (acoplamento medido):** o Selenium é o engine **mais completo** e está fundo no projeto — ~15 arquivos. As features **exclusivas de Selenium** são as mais valiosas: fluxo de login (`login.py`, `login_flow.py`), resolvers de challenge por IMAP (`challenge_resolvers.py`), `scroll_loop.py`, `modal_interaction.py`, `worker_pool.py`, `get_profile` (exige `_driver`), `diagnostics.py` e o **undetected-chromedriver** (modo furtivo, só-Selenium, adicionado no commit `e25c6a9`). O `PlaywrightEngine` já existe, porém é secundário e mais fino.

**Decisão: NÃO fazer rip-and-replace.** Motivos:
1. **Estratégico:** o roadmap rebaixa **todos** os engines web (Selenium/Playwright/httpx) a *fallback tier 3*. O destino é `MobileApiEngine` + `AndroidUiEngine`. Trocar um fallback por outro é ROI baixo.
2. **Custo/risco:** portar login + IMAP + stealth + scroll/modal + worker_pool é superfície enorme, com alto risco de regressão, e fere o princípio nº 5 ("reaproveitar o InstaT").
3. **Stealth:** o undetected-chromedriver é a aposta anti-detecção atual e é só-Selenium; o stealth do Playwright é tido como mais fraco contra detecção sofisticada.
4. **Cascata:** `BaseEngine` existe para **não** precisar escolher um engine único.

**Onde o Playwright é genuinamente melhor (e cabe):** para a **Fase 11** (paralelismo + sessão), `storage_state` (cookies + localStorage num JSON) e um `BrowserContext` por conta dão **isolamento e persistência multi-conta mais limpos** que a mexida manual de cookies do Selenium. Então a ação recomendada é **adicionar** o Playwright como opção de primeiro nível **no caminho paralelo da Fase 11** (medido contra o Selenium, com o `SessionStore` capaz de ler/gravar `storage_state`), **sem remover** o Selenium — que segue como fallback e dono do stealth/login/IMAP. O esforço grande vai para os engines mobile, não para trocar engine web.
...
3. **Pré-análise:** transporte do `MobileApiEngine` sobre `curl_cffi.requests.Session` com fingerprint de `instat/mobile/fingerprints/<app_version>.json`; **servidor TLS local de teste** que devolve o JA4/Akamai recebido (sem depender de site externo). **Print:** Wireshark lado a lado, app × engine; sem conta.
...
3. **Pré-análise:**
   ...
   - **Engine web do caminho paralelo:** Selenium (cookies via `add_cookie`) **ou** Playwright (`storage_state` + `BrowserContext` por conta, isolamento mais limpo) — medir os dois; **não** substituir o Selenium (§3.2).
...
**TLS:** [curl_cffi](https://github.com/lexiforest/curl_cffi) · [fingerprint customizado](https://curl-cffi.readthedocs.io/en/latest/impersonate/customize.html) · [targets](https://curl-cffi.readthedocs.io/en/stable/impersonate/targets.html) · [tls-client](https://github.com/bogdanfinn/tls-client)
